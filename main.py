...
class LingxiPlugin(Star):
    ...
    主动发言发起话题、分享资讯，精力系统与心流状态机把握参与时机，
    五路退却信号与静默作息保证知趣收声。支持 Telegram 和 QQ (aiocqhttp) 平台。

    核心机制：
    - 维护独立消息缓冲区，记录群内所有消息（包括未 @ 机器人的）
    - 命中名称时设置 is_at_or_wake_command = True，让核心管道处理
    - 通过 on_llm_request 钩子注入群聊上下文，使 LLM 了解完整对话氛围
    - 提供 11 条调试指令：状态/缓冲区/群组/精力/心流/防抖/Token 查看、缓冲区清理、主动发言触发与指标
    - 自动定期清理过期缓冲区数据
    - 精力系统：控制机器人回复频率，避免过度参与
    - 心流状态机：根据群聊活跃度在四态间流转，动态调整唤醒概率
    - 主动发言引擎：定时发起话题、订阅并分享 RSS 资讯
    - 五路退却信号：无人回应/被忽略/被否定/精力耗尽/作息静默时知趣收声
    - 冷场救场：群聊冷场时主动参与
    ...
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        categories_str = proactive_cfg.get(
            ...
            # v2.1.1：新增 游戏/数码科技/开发者 三个独立类别
            "分享想法|提问讨论|回忆过去|关注某人|活跃气氛|科技资讯|游戏八卦|沙雕新闻|热点事件|游戏|数码科技|开发者",
        )
    ...
    _TOPIC_CATEGORY_PROMPTS = {
        ...
        # v2.1.1 新增 3 个独立类别
        "游戏": "分享一条游戏行业的新消息或趣闻，用你的风格说说看",
        "数码科技": "分享一条数码科技领域的新鲜事，用你的风格转述并加上自己的看法",
        "开发者": "分享一条开发者社区或开源领域的动态，用你的风格说说这事儿",
    }

    # 资讯类话题集合（需调用 Fetcher 获取外部资讯）
    _NEWS_CATEGORIES = {"科技资讯", "游戏八卦", "沙雕新闻", "热点事件", "游戏", "数码科技", "开发者"}
