...
- **3 个独立话题类别**：游戏（机核/游研社）、数码科技（少数派/Solidot）、开发者（开源中国/HelloGitHub/GitHub Blog），全部直连源实测可达
- `proactive_topic_categories` 默认值从 9 类扩充到 **12 类**

### 文档

- CHANGELOG 全面重写：移除内部流程叙述，改为开发者视角的版本变更说明
- README 中英文重写：措辞和句子结构优化，去除术语堆砌
- 插件描述重写：平台兼容信息移至末尾
...
- **语音替换文字后出现空气泡**：开启「语音替换文字」后，语音消息后会跟一条看不见内容的空气泡。原因是末段文字被替换后，插件用零宽空格占位防止框架发空消息，但零宽空格不被框架识别为空白内容，仍会作为一条消息发出。现在直接留空消息链，框架会自动跳过，后续的去重和记忆记录不受影响。

## [2.1.0] - 2026-09-19

### 新增

- **分段语音**：新增 `tts_mode` 配置项（默认 `last` 保持原有行为）。设为 `per_segment` 时，每个分段独立判定是否配音；设为 `one_per_reply` 时，每次回复只选一段配音。需先在 AstrBot 官方设置中开启 TTS。
- **语音概率自管**：新增 `tts_probability` 配置项（默认 0.3），不再依赖 AstrBot 官方的触发概率设置。
- **主动发言语音**：主动发言的分段现在同样支持配音（此前主动发言始终没有语音）。

### 其他

- 新增 `tts_max_per_reply`（单次回复配音段数上限）与 `tts_min_chars`（过短分段不配音）。
- Telegram 等远程平台自动通过 file_service 发送语音（此前仅 NapCat 同机部署可用）。

## [2.0.4] - 2026-09-19

### 修复

- **资讯推送配图与正文不匹配**：此前图片固定使用第一条资讯的配图，但 Bot 实际评论的可能是其他资讯，导致图文不符。现在会根据回复内容与各条资讯的匹配程度选取最相关的配图，匹配度不足时不配图。

### 其他

- 插件描述重写：平台兼容性信息移至末尾，改为更平实的表达。

## [2.0.3] - 2026-09-18

### 修复

- **单群参数覆盖不完整**：静默时段（`proactive_quiet_hours`）虽然在面板中可以配置，但实际不会被应用到对应群。现在已修复，海外群可以正常设置独立的静默时段。同时补齐了轻量回应的概率/冷却/每小时上限三个单群覆盖字段。
- **资讯跨天重复推送**：已发送资讯的记录仅保存在内存中，插件重载后丢失，导致同一条资讯隔天被重复推送。现在会将发送记录持久化到磁盘（保留 14 天），重载后不再重复。
- **部分旧配置升级后资讯话题缺失**：从旧版本升级时如果缺少话题类别配置项，会回退到不含资讯类的旧版默认值。现在回退值与当前版本对齐，不会再丢失资讯类话题。
- **Bot 名称配置缺失时唤醒失效**：`bot_name` 配置缺失时回退值从空字符串改为 `Bot`，避免名称唤醒功能静默失效。
- 5 个主动发言参数缺少最小值校验，现已补齐，面板保存时会正确拦截超出范围的值。

### 其他

- 修复了与其他插件配合使用时，灵犀的错误处理逻辑可能覆盖其他插件注入内容的问题。
- `cascade_upgrade_enabled` 配置项已无实际作用（对应功能在早期版本已移除），标注为已弃用。

### 新增

- **资讯推送附带首图**（`fetcher_send_first_image`，默认关闭）：开启后，资讯类主动发言会尝试附带文章配图。支持 RSS 内嵌图片提取，带防盗链下载兜底，无图或下载失败时自动降级为纯文本。

## [2.0.2] - 2026-09-18

### 修复

- **面板修改参数后不生效**：Web 配置面板保存参数后，正在运行的插件不会立即应用新值，需要手动重载插件才能生效。现在保存成功后会自动触发热重载，参数修改即时生效。此前面板的热重载调用在失败时不会给出任何提示，用户会误以为已生效——现在重载失败会明确提示。
- **应用预设后不生效**：预设功能从上线起就缺少热重载步骤，应用预设后行为不变。现已修复。

## [2.0.1] - 2026-09-17

### 修复

- **日志系统统一**：将全部模块的日志输出统一为 AstrBot 标准 logger，不再混用 Python 标准 logging（混用会导致部分日志不出现在 AstrBot 日志中，排查困难）。
- **数据文件位置规范化**：将 `proactive_state.json`（主动发言状态）和 `presets.json`（用户预设）从插件目录迁移到 AstrBot 标准数据目录 `data/plugin_data/`，避免市场更新插件时数据丢失。旧文件自动迁移，无需手动处理。

## [2.0.0] - 2026-09-17

版本规范化与品牌重构。v1.4.4 实际合并了内部开发序列 v1.8.4 与 v1.9.0 ~ v1.9.8 共 10 个大版本的演进（主动发言引擎、Fetcher 资讯模块、五路退却、静默作息、Web 配置面板等），以补丁号发布不符合语义化版本规范。本版本不含功能代码变更，将对外版本号正名为 2.0.0，并完成全部对外文本的定位重构：从"被动唤醒插件"升级为"会主动、知进退、有作息的群友型 Bot 节律引擎"。
...
- `README.md` / `README_EN.md` 全量重写：三支柱叙事（会主动 / 知进退 / 可调校）、19 条卖点、功能一览勘误与扩充、调试指令 4→11 条、FAQ 持久化条目修正并新增主动发言相关条目、mermaid 工作原理图补主动发言回路
