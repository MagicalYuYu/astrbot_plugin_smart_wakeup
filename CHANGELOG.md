...
- **"昵称:内容"引用模式导致 LLM 发言人识别错乱**：修复当用户消息以"昵称:"开头（如 `gamer:服务器不插网线...`）时，上下文格式 `[MagicalYu]: gamer:xxx` 中的双重冒号结构使 LLM 误判 `gamer` 为实际发言者，导致回复对象错误的问题。在 `_format_context` 和 `_format_conversation_memory` 中增加歧义消除：检测到消息内容以"昵称:"模式开头时，用引号包裹内容（`「gamer:xxx」`），使 LLM 明确区分发送者标注与被引用/转述的内容
