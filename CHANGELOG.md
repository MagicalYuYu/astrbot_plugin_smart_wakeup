...
- **修复概率唤醒语义重复回复**：两次独立的概率唤醒触发两次 LLM 调用时，LLM 基于相似上下文可能生成语义重复的回复（表述不同但实质内容相近）。新增语义去重机制：在输出阶段（`on_decorating_result`）计算新回复与 BOT 最近一次回复的字符 bigram Jaccard 相似度，120秒窗口内相似度≥0.55时抑制新回复。仅对非直接呼叫场景生效，直接呼叫（名称触发/关键词触发/回复BOT）不受影响
