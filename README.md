...
大多数群机器人只有两种状态：被 @ 才应，不 @ 装死。灵犀让 Bot 成为真正的群友——它会主动发起话题、分享刚发生的资讯，聊嗨了全程在场，被无视时知趣收声，深夜到点睡觉。143 项参数与专属可视化面板，让它在你的每个群里都是不同的性格。
...
- 🔧 **143 个可调参数** — 14 个配置分组，从精力恢复到话题权重全部可调
...
- **分组标签** — 143 项参数按 14 个配置分组组织，不再滚屏找参数
...
143 项参数均有合理默认值，开箱即用。详细配置说明请参阅 [使用指南](docs/usage_guide.md)。
